...
function Requests() {
  ...
  const [currentRequest, setCurrentRequest] = useState<{
    id: string;
    item: any;
    action: "view" | "sms" | "timeline" | "rescheduler" | "transfer";
  } | null>(null);
  ...
  useEffect(() => {
    ...
    if (searchToken) {
      filtered = requests.filter((r) =>
        r.token?.toLowerCase().includes(searchToken.toLocaleLowerCase())
      );
    }
  ...
  const columns = [
    { key: "token", label: "Token" },
    {
      key: "outlet",
      label: "Outlet",
      render: (request: any) => `${request.outlet.name}`,
    },
    {
      key: "type",
      label: "Type",
      render: (request: any) => (GasTypesValues as any)[request.type],
    },
    ...
    if (isOutletManager) {
      return [
        ...
        },
        {
          ...
          onClick: handleResceduleRequest,
          condition: (item: any) => item.status === RequestStatus.EXPIRED,
        },
        {
      ...
      return [
        {
          label: "Cancel Request",
          onClick: handleCancelRequest,
          condition: (item: any) => item.status === RequestStatus.PLACED,
        },
        { label: "View Timeline", onClick: handleViewTimeline },
      ];
    } else {
  ...
  return (
    <AppLayout>
      ...
      >
          ...
          <Modal.Content>
            {isOutletManager && (
              <div className="mb-2">
                <CheckBox
                  ...
                  onChange={(value: boolean) => setIsExistingCustomer(value)}
                />
              </div>
            )}
            <div className="mb-2">
              {isOutletManager ? (
                ...
                <Select
                  ...
                  options={outlets.map((outlet) => ({
                    ...
                    label: `${outlet.name} (${outlet.district})`,
                  }))}
            ...
            <div className="mb-2">
              <Select
                ...
                options={Object.keys(GasTypes).map((key: string) => ({
        ...
        {currentRequest && currentRequest.action === "view" && (
          <ViewCustomer
            id={currentRequest.id}
            onClose={() => setCurrentRequest(null)}
          />
        )}
        {currentRequest && currentRequest.action === "sms" && (
          <SendSMS
            id={currentRequest.id}
            onClose={() => setCurrentRequest(null)}
          />
        ...
        {currentRequest && currentRequest.action === "rescheduler" && (
          <Rescheduler
            id={currentRequest.id}
            min={currentRequest.item?.deadlineForPickup}
            onClose={() => setCurrentRequest(null)}
          />
        )}

        {currentRequest && currentRequest.action === "transfer" && (
          <Transfer
            id={currentRequest.id}
            onClose={() => setCurrentRequest(null)}
          />
        )}
        {currentRequest && currentRequest.action === "timeline" && (
          <TimelineView
            events={currentRequest.item.timelines}
            onClose={() => setCurrentRequest(null)}
          />
        )}
      </div>
...
export default AuthRoleCheck(Requests, {
  roles: [UserRole.BUSINESS, UserRole.CUSTOMER, UserRole.OUTLET_MANAGER],
});
